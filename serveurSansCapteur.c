...
#define ADRESSE_IP_SERVEUR_MQTT "192.168.1.195"
#define KEEPALIVE 0
...
int gererSecurite(MYSQL *pMysql, struct modules module);
...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		printf("usage : %s <Trame sans espace>\n", argv[0]);
		return -1;
	}
	
	while (argv[1][i] != '\0') {
		if (argv[1][i] >= 0x41 && argv[1][i] <= 0x46) {
			characterLu = argv[1][i] - 55;
		}
		else if (argv[1][i] >= 0x30 && argv[1][i] <= 0x39) {
			characterLu = argv[1][i] - 48;
		} else if (argv[1][i] >= 0x61 && argv[1][i] <= 0x66) {
			characterLu = argv[1][i] - 87;
		}
		if ((i%2) == 0) {
			trame[i/2] = (characterLu<<4);
		} else {
			trame[i/2] += characterLu;
		}
		i++;
	}
